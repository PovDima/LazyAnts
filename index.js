...
async function startRecursive(sum = 0, ind = 0) {

  const fetchUsers = () => {
    let arr = [];

    for (let i = ind; i < ind + 10; i++) {
      arr.push(processUser(users[i].id));
    }
  ...
  sum += (await Promise.all(fetchUsers())).reduce((res, elem) => res += elem, 0);

  if (ind < users.length) {
    return startRecursive(sum, ind);
  } else {
    return new Promise((resolve, reject) => {
      resolve(sum);
    })
